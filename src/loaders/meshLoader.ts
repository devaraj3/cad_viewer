...
import { createStainlessSteelMaterial } from '../render/viewer'
import {
...
export { DEFAULT_CAD_TOPOLOGY_AVAILABILITY } from '../core/exact-cad-topology'
...
function applyStainlessSteelMaterialOverrides(root: any, doubleSide = false) {
  ...
  root.traverse((child: any) => {
    ...
    } catch { /* ignore any weird loader material shapes */ }
  })
}
...
function mergeFromObject(root: any) {
  ...
  if (
    ...
    } catch (_e) { /* ignore for non-manifold */ }
  }
  return merged
}
...
function isCADExt(ext: string): ext is CADExt {
  return (
    ext === 'step' || ext === 'stp' || ext === 'iges' || ext === 'igs' || ext === 'brep'
  )
}
...
function isCadExactExportFormat(format: string): format is CadExactExportFormat {
...
function normalizeSheetMetalMeta(raw: any): SheetMetalMeta {
  if (!raw || typeof raw !== 'object') {
    return { isAssembly: false, isSheetMetal: false, reason: 'analysis_failed' }
  }
...
function normalizeWorkerCapabilities(raw: unknown): WorkerCapabilities {
  if (!raw || typeof raw !== 'object') return { ...DEFAULT_WORKER_CAPABILITIES }

  const caps = raw as WorkerCapabilities
  const supportedExactCadFormats = Array.isArray(caps.supportedExactCadFormats)
    ? caps.supportedExactCadFormats.filter((entry: unknown): entry is CadExactExportFormat =>
        isCadExactExportFormat(String(entry)),
      )
    : []

  const exactCadPartExport = !!caps.exactCadPartExport && supportedExactCadFormats.length > 0
  const exactCadTopology = !!caps.exactCadTopology

  return { exactCadPartExport, exactCadTopology, supportedExactCadFormats }
}
...
function normalizeCadTopologyAvailability(raw: unknown): CadTopologyAvailability {
  if (!raw || typeof raw !== 'object') return { ...DEFAULT_CAD_TOPOLOGY_AVAILABILITY }
  ...
  return { exact: !!availability.exact, reason, message, runtimeSymbols }
}
...
function normalizeCadRoot(root: any, meshCount: number): CadAssemblyNode {
  ...
  const rawMeshes = Array.isArray((root as any).meshes) ? (root as any).meshes : []
  const meshes = rawMeshes
    .map((idx: any) => Number(idx))
    .filter((idx: number) => Number.isInteger(idx))

  const rawChildren = Array.isArray((root as any).children) ? (root as any).children : []
  const children = rawChildren.map((child: any) => normalizeCadRoot(child, meshCount))
  ...
  return new Promise<CadAssemblyWithTopologyLoadResult>((resolve, reject) => {
    const handle = (
      ...
      if (!data.ok) {
        ...
        // Fallback: tessellate in parts mode without topology
        ...
        const fallbackHandle = (
            ...
            reject(
              new Error(
                `${topologyError} Fallback tessellation failed: ${
                  'error' in fallbackData && typeof fallbackData.error === 'string'
                    ? fallbackData.error
                    : 'OpenCascade error'
                }`,
              ),
          ...
          if (!('mode' in fallbackData) || fallbackData.mode !== 'parts') {
            reject(
              new Error(`${topologyError} Fallback tessellation did not return parts data.`),
            )
            return
          }

          const built = buildCadAssemblyScene(fallbackData.meshes, fallbackData.root)
          resolve({
            ...built,
            ...
            topologyAvailability: {
              exact: false,
              reason: topologyError.includes('Missing required OCCT runtime export') ||
        ...
        worker.postMessage(
          {
            id: fallbackId,
            type: 'tessellate',
            payload: { buffer: fallbackBuffer, ext, mode: 'parts' },
          } as TessReq,
          [fallbackBuffer],
        )
        return
      ...
      if ('type' in data && data.type === 'tessellate_with_topology') {
        ...
        resolve({
          ...
          topologyAvailability: normalizeCadTopologyAvailability(data.topologyAvailability),
        })
        return
      }
...
export async function loadMeshAssemblyAsObject3D(file: File | string): Promise<THREE.Object3D> {
  const { fileObj, ext } = await resolveInputFile(file)
  ...
  if (!object) throw new Error('No object hierarchy found in file')
...
export async function analyzeCadSheetMetal(
  ...
  const { fileObj, ext } = await resolveInputFile(file)
  if (!isCADExt(ext)) {
    return { isAssembly: false, isSheetMetal: false, reason: 'not_brep_source' }
  }
  ...
  return new Promise<SheetMetalMeta>((resolve, reject) => {
    ...
    worker.addEventListener('message', handle as any)
    worker.postMessage(
      { id, type: 'analyze_sheetmetal', payload: { buffer: buf, ext } } as AnalyzeSheetMetalReq,
      [buf],
    )
  })
}
...
export async function unfoldCadSheetMetal(
  ...
  if (!isCADExt(ext)) {
    return {
      meta: { isAssembly: false, isSheetMetal: false, reason: 'not_brep_source' },
  ...
  return new Promise<{ flat: THREE.BufferGeometry; meta: SheetMetalMeta }>((resolve, reject) => {
    const handle = (e: MessageEvent<UnfoldSheetMetalOk | TessErr>) => {
      ...
      resolve({ flat, meta: normalizeSheetMetalMeta(data.meta) })
    }
    ...
    worker.postMessage(
      {
        id,
        type: 'unfold_sheetmetal',
        payload: { buffer: buf, ext, kFactor: opts.kFactor, thicknessOverrideMM: opts.thicknessOverrideMM },
...
export async function exportCadPartExact(
  ...
  return new Promise<{ format: CadExactExportFormat; bytes: Uint8Array }>((resolve, reject) => {
    const handle = (e: MessageEvent<ExportPartOk | TessErr>) => {
      ...
      const bytes =
        data.bytes instanceof Uint8Array ? data.bytes : new Uint8Array(data.bytes as any)
      if (bytes.length === 0) {
        reject(new Error('Exact CAD export returned an empty payload.'))
        return
      }

      const outFormat = isCadExactExportFormat(data.format) ? data.format : payload.format
...
export async function getWorkerCapabilities(
  ...
  return new Promise<WorkerCapabilities>((resolve) => {
    ...
    const handle = (e: MessageEvent<GetWorkerCapabilitiesOk | TessErr>) => {
      ...
      if (!data.ok || (data as any).type !== 'get_worker_capabilities') {
        finalize({ ...DEFAULT_WORKER_CAPABILITIES })
        return
      }

      finalize(normalizeWorkerCapabilities((data as GetWorkerCapabilitiesOk).capabilities))
    }
...
// ─── Legacy flat-mesh loader (kept for STL/OBJ/3MF/glTF drop-in use) ─────────
