...
import type { ModelSession, PartDescriptor } from '../core/model-session'
import { findPartRootByKey, resolveObjectByPath } from '../core/model-session'

export type PartExportFormat = 'stl' | 'obj' | 'glb' | 'step' | 'iges' | 'brep'
...
function isMeshExportFormat(format: PartExportFormat): format is 'stl' | 'obj' | 'glb' {
...
export function inferDefaultPartExportFormat(session: ModelSession): PartExportFormat {
  return session.kind === 'cad' ? 'step' : 'glb'
}

function supportsExactCadFormat(caps: WorkerCapabilities, format: CadExactExportFormat): boolean {
...
export function getWorkingPartExportPlan(
  ...
  if (ext === 'obj') return { mode: 'mesh', format: 'obj' }
  if (ext === 'gltf' || ext === 'glb' || ext === '3mf') return { mode: 'mesh', format: 'glb' }
...
export function resolveCadPartAsObject3D(
  ...
  sourceRoot.traverse((node) => {
    ...
    if (!meshByCadIndex.has(cadMeshIndex)) meshByCadIndex.set(cadMeshIndex, node)
  })
...
export function bakeObjectHierarchyToWorldGeometry(source: THREE.Object3D): THREE.Group {
...
async function exportAsStlBinary(object: THREE.Object3D): Promise<SerializedPartPayload> {
  ...
  if (serialized instanceof DataView) {
    const sourceBytes = new Uint8Array(serialized.buffer, serialized.byteOffset, serialized.byteLength)
    const copied = new Uint8Array(serialized.byteLength)
    copied.set(sourceBytes)
    return { data: copied.buffer, mime: meshMimeType('stl') }
  }
  if (serialized instanceof ArrayBuffer) return { data: serialized, mime: meshMimeType('stl') }
  if (typeof serialized === 'string') return { data: serialized, mime: meshMimeType('stl') }
...
async function exportAsObj(object: THREE.Object3D): Promise<SerializedPartPayload> {
  ...
  if (typeof serialized !== 'string') throw new Error('Failed to serialize OBJ payload.')
...
async function exportAsGlb(object: THREE.Object3D): Promise<SerializedPartPayload> {
  ...
  const serialized = await new Promise<ArrayBuffer>((resolve, reject) => {
    exporter.parse(
      ...
      (error) => {
        reject(error instanceof Error ? error : new Error('GLB export failed unexpectedly.'))
      },
...
async function serializePartObject(
  ...
  if (!isMeshExportFormat(format)) throw new Error('Exact CAD formats are not mesh-serialized.')
  if (format === 'stl') return exportAsStlBinary(object)
  if (format === 'obj') return exportAsObj(object)
...
function resolveCadSourceExt(
  ext: string,
): 'step' | 'stp' | 'iges' | 'igs' | 'brep' | null {
...
async function exportDescriptorAsExactCad(
  ...
  download(result.bytes.buffer as ArrayBuffer, fileName, exactCadMimeType(result.format))
...
export async function exportSelectedPartFromSession(
  ...
  try {
    ...
    try {
      const fallbackResult = await exportDescriptorAsMesh(session, descriptor, 'stl', download)
      return { ...fallbackResult, fallbackFrom: plan.format }
    } catch (meshError) {
