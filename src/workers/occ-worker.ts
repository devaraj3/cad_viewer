...
async function init() {
  ...
  if (!origin || origin === 'null') {
    console.warn('OCC Worker: Base origin is null, falling back to relative paths.')
  }
  ...
  try {
    ...
    // Fallback to v1 if v2 is unavailable
    try {
      ctx.importScripts('/occ/occt-import-js.js')
    } catch (e2: any) {
      throw new Error(`Failed to load OpenCascade script. Error: ${e.message}`)
    }
  }
  ...
  console.log('[OCCT] AnalyzeSheetMetal:', typeof (occt as any).AnalyzeSheetMetal)
  console.log('[OCCT] ExportPart:', typeof (occt as any).ExportPart)
  console.log('[OCCT] TessellateWithTopology:', typeof (occt as any).TessellateWithTopology)
...
function getAvailableTopologyRuntimeSymbols(mod: any): string[] {
  ...
  const symbols = Object.keys(mod).filter(
    (entry) =>
      typeof mod?.[entry] === 'function' && /topolog|tessellat|extract/i.test(entry),
  )
...
function buildMissingTopologyRuntimeMessage(runtimeSymbols: string[]): string {
  const symbolList = runtimeSymbols.length > 0 ? runtimeSymbols.join(', ') : '<none>'
  return [
...
function collectTopologyTransferables(topology: CadTopologyResult | null): Transferable[] {
  if (!topology) return []
...
function isCadExactExportFormat(format: unknown): format is CadExactExportFormat {
...
function getWorkerCapabilities(mod: any): WorkerCapabilities {
  ...
  return {
    exactCadPartExport,
    exactCadTopology,
    supportedExactCadFormats: exactCadPartExport ? ['step', 'iges', 'brep'] : [],
  }
}
...
ctx.onmessage = async (e: MessageEvent<any>) => {
  ...
  if (type === 'tessellate') {
    try {
      ...
      const res = readCadResult(mod, ext, u8, linearDeflection, angularDeflection)
      ...
      if (effectiveMode === 'parts') {
        ...
        ctx.postMessage({ id, ok: true, mode: 'parts', root, meshes } as TessPartsOk, transferables)
        return
      }
      ...
      for (const m of res.meshes as any[]) {
        ...
        for (let k = 0; k < i.length; k++) {
          idx[idxOffset + k] = (i[k] as number) + indexOffsetBonus
        }
  ...
  if (type === 'tessellate_with_topology') {
    try {
      ...
      if (!topologyExtractionResult || topologyExtractionResult.success === false) {
        throw new Error(
          topologyExtractionResult?.error || 'Exact topology extraction failed in runtime.',
        )
      }
      ...
      const { root, meshes, transferables } = packTessPartsPayload(topologyExtractionResult)
      const topologyAvailability: CadTopologyAvailability = availableTopologyAvailability(mod)
  ...
  if (type === 'unfold_sheetmetal') {
    try {
      ...
      if (!result || result.success === false) {
        throw new Error(result?.error || 'Failed to unfold sheet metal geometry')
      }
