...
import type { CadAssemblyLoadResult, CadAssemblyNode } from '../loaders/meshLoader'
...
function containsExportableMesh(object: THREE.Object3D): boolean {
  ...
  object.traverse((node) => {
    if (found) return
    if (isExportableMeshNode(node)) found = true
  })
  return found
}
...
function cloneMeshWithResources(source: THREE.Mesh): THREE.Mesh {
  ...
  if (source.geometry) cloned.geometry = source.geometry.clone()
  if (source.material) cloned.material = cloneMaterialLike(source.material)
...
function cloneObject3DWithResources(source: THREE.Object3D): THREE.Object3D {
  ...
  source.traverse((node) => {
    if (isExportableMeshNode(node)) sourceMeshes.push(node)
  })
  cloned.traverse((node) => {
    if (isExportableMeshNode(node)) clonedMeshes.push(node)
  })
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (srcMesh.geometry) dstMesh.geometry = srcMesh.geometry.clone()
    if (srcMesh.material) dstMesh.material = cloneMaterialLike(srcMesh.material)
...
export function findPartRootByKey(
  ...
  for (const child of root.children) {
    const childKey =
      typeof child.userData?.__partKey === 'string' ? child.userData.__partKey : null
...
function buildCadPartDescriptors(
  ...
  const groupedByPartId = new Map<string, { meshIndices: Set<number>; preferredName?: string }>()
  ...
  const hydrateFromTree = (node: CadAssemblyNode) => {
    ...
    if (partId) {
      ...
      for (const idx of meshIndices) group.meshIndices.add(idx)
      if (!group.preferredName) {
        group.preferredName = normalizePartName(node.name, '')
      }
    }
    for (const child of node.children ?? []) hydrateFromTree(child)
  }
  ...
  for (const entry of entries) {
    ...
    for (const idx of normalized) assigned.add(idx)
  ...
  // Fallback for runtimes that don't expose native part ids
  for (let meshIndex = 0; meshIndex < meshCount; meshIndex++) {
...
function attachPartMetadata(object: THREE.Object3D, descriptor: PartDescriptor): void {
...
function createPartNameFromObject(object: THREE.Object3D, fallbackIndex: number): string {
...
export function createMeshModelSession(
  ...
  const sourcePartRoots = hasTopLevelChildren ? sourceObject.children : [sourceObject]
  const displayPartRoots = hasTopLevelChildren ? displayObject.children : [displayObject]
  ...
  sourcePartRoots.forEach((sourcePartRoot, index) => {
    ...
    const descriptor: MeshPartDescriptor = { key, kind: 'mesh', name, objectPath }
    partMap.set(key, descriptor)

    attachPartMetadata(sourcePartRoot, descriptor)
    const displayPartRoot = displayPartRoots[index]
    if (displayPartRoot) attachPartMetadata(displayPartRoot, descriptor)
  })
...
export function listSessionDisplayParts(session: ModelSession): SessionDisplayPart[] {
  ...
  const pushPart = (object: THREE.Object3D) => {
    const key =
      typeof object.userData?.__partKey === 'string' ? object.userData.__partKey : null
    if (!key) return
